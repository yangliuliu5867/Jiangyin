...
ros::Publisher Lcircle_pos_pub,Rcircle_pos_pub;
ros::Publisher task2Pospub,task3Pospub;
ros::Publisher circle1pub, circle2pub;
...
void estDoorPos(pcl::PointCloud<pcl::PointXYZ> cloud_in,int task){
    ...
    int planeLeftIndex = 0;
    int planeRightIndex = 0;
    ...
    if(task == 2){
        for(int i =0;i<cloud.size();i++){
            ...
            if(x<cloud_xmin) {cloud_xmin=x;}
            if(x>cloud_xmax) {cloud_xmax=x;}
            if(y<cloud_ymin) {cloud_ymin=y;planeRightIndex=i;}
            if(y>cloud_ymax) {cloud_ymax=y;planeLeftIndex=i;}
        }
        ...
        double detPlane_width = std::sqrt(std::pow((cloud_xmax-cloud_xmin),2)+std::pow(cloud_ymax-cloud_ymin,2));
        double detPlane_angle = std::atan((cloud.points[planeRightIndex].x-cloud.points[planeLeftIndex].x)/(cloud.points[planeLeftIndex].y-cloud.points[planeRightIndex].y));
        std::cout<<"plane_width: "<<detPlane_width<<std::endl;
        std::cout<<"plane_angle: "<<detPlane_angle<<std::endl;
        ...
        //     std::cout<<"WallPos: "<<"X: "<<planeLeftPosX <<" Y: "<<planeLeftPosY +0.4<<std::endl;
        //     // geometry_msgs::PoseStamped circle_pose;
        //     // circle_pose.header.stamp = ros::Time::now();
        //     // circle_pose.header.frame_id = "world";
        //     // circle_pose.pose.position.x = planeLeftPosX + doorWidthPos_NearLeft*sin(detPlane_angle);
        ...
        //     circle_pose.header.stamp = ros::Time::now();
        //     circle_pose.header.frame_id = "world";
        //     circle_pose.pose.position.x = planeLeftPosX ;
        //     circle_pose.pose.position.y = planeLeftPosY +0.4 - 0.1;
        //     circle_pose.pose.position.z = doorHightPos_NearFloor;
        //     circle_pose.pose.orientation.x = 0;
        //     circle_pose.pose.orientation.y = 0;
        //     circle_pose.pose.orientation.z = 0;
        ...
        //     circle_pose.header.stamp = ros::Time::now();
        //     circle_pose.header.frame_id = "world";
        //     circle_pose.pose.position.x = planeLeftPosX + 6.0;
        //     circle_pose.pose.position.y = planeLeftPosY - 1.39;
        //     circle_pose.pose.position.z = doorHightPos_NearFloor;
        //     circle_pose.pose.orientation.x = 0;
        //     circle_pose.pose.orientation.y = 0;
        //     circle_pose.pose.orientation.z = 0;
        ...
        for(int i =0;i<cloud.size();i++){
            double x = cloud.points[i].x;
            double y = cloud.points[i].y;
            if(x<cloud_xmin) {cloud_xmin=x;planeLeftIndex=i;}
            if(x>cloud_xmax) {cloud_xmax=x;planeRightIndex=i;}
            if(y<cloud_ymin) {cloud_ymin=y;}
            if(y>cloud_ymax) {cloud_ymax=y;}
        }
        ...
        double detPlane_width = std::sqrt(std::pow((cloud_xmax-cloud_xmin),2)+std::pow(cloud_ymax-cloud_ymin,2));
        double detPlane_angle = std::atan((cloud.points[planeLeftIndex].y-cloud.points[planeRightIndex].y)/(cloud.points[planeRightIndex].x-cloud.points[planeLeftIndex].x));
        std::cout<<"plane_width: "<<detPlane_width<<std::endl;
        std::cout<<"plane_angle: "<<detPlane_angle<<std::endl;
        ...
        if(detPlane_width>0.95*wallWidthTrue && detPlane_width <1.05*wallWidthTrue){
            ...
            std::cout<<"WallPos: "<<"X: "<<planeLeftPosX - doorWidthPos_NearLeft*cos(detPlane_angle)<<" Y: "<<planeLeftPosY + doorWidthPos_NearLeft*sin(detPlane_angle)<<std::endl;
            // geometry_msgs::PoseStamped circle_pose;
            // circle_pose.header.stamp = ros::Time::now();
            // circle_pose.header.frame_id = "world";
...
std::vector<Eigen::Vector4d> ransacFitPlane(std::vector<Point>& points_input,pcl::PointCloud<pcl::PointXYZ> points_pcl_input, int iterations, double threshold,int GoalInliners) {
    ...
    *cloud_input = pclCloud[0];
    ...
    estDoorPos(*cloud_clustered,currentTask);
...
// RANSAC平面拟合
Eigen::Vector4d ransacFitOnePlane(std::vector<Point>& points_input,pcl::PointCloud<pcl::PointXYZ> points_pcl_input, int iterations, double threshold,int GoalInliners,int task) {
    ...
    pcl::PointCloud<pcl::PointXYZ> planePclCloud;
    // pcl::PointCloud<pcl::PointXYZ> cloud1_out;
    // cloud1.resize(2000);
    ...
    while (it != points_pcl.end()) {
        // 计算每个点到平面的距离
        if (distancePointToPlane(*it, bestPlaneNow) < threshold) {
            // 使用erase方法删除当前元素，并自动更新迭代器
            // pcl::PointXYZ point;
            // point.x = it->x;
            // point.y = it->y;
            // point.z = it->z;
            planePclCloud.push_back(*it);
        }
        ++it;
    }
    std::cout<<"bestInliers: "<<bestInliers<<std::endl;
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_input(new pcl::PointCloud<pcl::PointXYZ>);
    *cloud_input = planePclCloud;
    pointPublish(*cloud_input,pointCloud4pub);
    // std::cout<<"points_num_nowin11111: "<<points_pcl.size()<<std::endl;
    //点云聚类
    pcl::search::KdTree<pcl::PointXYZ>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZ>);
    tree->setInputCloud(cloud_input);
    std::vector<pcl::PointIndices> cluster_indices;
    ...
    DBSCANKdtreeCluster<pcl::PointXYZ> ec;
    ec.setCorePointMinPts(5);
    ec.setClusterTolerance(0.3);//容忍距离
    ec.setMinClusterSize(10);//点云聚类的最小点数量
    ec.setMaxClusterSize(25000);
    ec.setSearchMethod(tree);
    ec.setInputCloud(cloud_input);
    ...
    //     std::cout<<"cluster_indices: "<<cluster_indices[i].indices.size()<<std::endl;
    // }
    // std::cout<<"points_num_nowin22222: "<<points_pcl.size()<<std::endl;
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); it++) {
        for(std::vector<int>::const_iterator pit = it->indices.begin(); pit != it->indices.end(); ++pit) {
            pcl::PointXYZ tmp;
            tmp.x = cloud_input->points[*pit].x;
    ...
    estDoorPos(*cloud_clustered,task);
    pointPublish(*cloud_clustered,pointCloud1pub);
    // if(cloudValidFlag[0]) pointPublish(pclCloud[0],pointCloud1pub);
    // if(cloudValidFlag[1]) pointPublish(pclCloudFilted[1],pointCloud2pub);
...
void ransacFitCircle(pcl::PointCloud<pcl::PointXYZ>::Ptr points, int task){
        ///存放拟合圆
...
void DoubleCircleFit(pcl::PointCloud<pcl::PointXYZ>::Ptr points){
    ...
    *cloud_in = *points;
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_after_sor(new pcl::PointCloud<pcl::PointXYZ>);
    sor.filter(*cloud_after_sor);
    pointPublish(*cloud_after_sor,pointCloud1pub);

    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_work(new pcl::PointCloud<pcl::PointXYZ>);
    *cloud_work = *cloud_after_sor;

    std::vector<Eigen::VectorXf> circle_models;
    std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> circle_clouds(2);

    for(int itr = 0; itr < 2; ++itr){
        if(cloud_after_sor -> size() < 10) break;
        ...
        seg.setRadiusLimits(0.5,0.6);//添加拟合圆的半径限制，防止拟合过大或过小的圆
        seg.setInputNormals(normals);//输入法向量
        seg.setInputCloud(cloud_work);//传入点集
        seg.segment(*inliers,*coefficients);//分割

        if(inliers->indices.empty()){
            std::cout<<"no plane detected:"<<itr<<std::endl;
            break;
        }

        Eigen::VectorXf model(7);
        for (int i=0;i<7;i++) model[i]=coefficients->values[i];
        circle_models.push_back(model);
        ...
        cloud_work.swap(cloud_left);
        
        extract.setNegative(false);
        extract.filter(*cloud_cir);
        circle_clouds[itr] = cloud_cir;

        if(itr == 0) {pointPublish(*cloud_cir,circle1pub);} 
        else {pointPublish(*cloud_cir,circle2pub);} 

    }
    if (circle_models.size() < 1) return;
            // 存储有效圆环的索引和位置
        std::vector<std::pair<int, float>> valid_circles; // <index, x_position>

        for (int i = 0; i < circle_models.size(); i++) {
            Eigen::VectorXf model = circle_models[i];
            if (abs(model[4]) > 0.95 && abs(model[5]) < 0.2 && abs(model[6]) < 0.2) {
                std::cout << "circle pos:" << model[0] << " " << model[1] << " " << model[2] << std::endl;
                std::cout << "circle size:" << model[3] << " " << model[4] << " " << model[5] << " " << model[6] << std::endl;
            if(i == 0){
                double A = model[4];
                double B = model[5];
                double C = model[6];
                double D = -A * model[0] - B * model[1] - C * model[2];
                ...
                // 保存有效圆环的索引和x坐标
                valid_circles.push_back(std::make_pair(i, model[1]));
            }
    }

    // 按x坐标排序（左小右大）
    std::sort(valid_circles.begin(), valid_circles.end(), 
    ...
    if(valid_circles.size() > 1 ){
        ...
        for (int i = 0; i < valid_circles.size(); i++) {
            ...
            circle_pose.header.stamp = ros::Time::now();
            circle_pose.header.frame_id = "world";
            circle_pose.pose.position.x = model[0];
            circle_pose.pose.position.y = model[1];
            circle_pose.pose.position.z = model[2];
            circle_pose.pose.orientation.x = 0;
            circle_pose.pose.orientation.y = 0;
            circle_pose.pose.orientation.z = 0;
            circle_pose.pose.orientation.w = 1;
            ...
            // int idx1 = valid_circles[0].first;
            // Eigen::VectorXf model1 = circle_models[idx1];

            // geometry_msgs::PoseStamped circle_pose1;
            // circle_pose1.header.stamp = ros::Time::now();
            // circle_pose1.header.frame_id = "world";
            // circle_pose1.pose.position.x = model1[0];
            // circle_pose1.pose.position.y = model1[1];
            // circle_pose1.pose.position.z = model1[2];
            // circle_pose1.pose.orientation.x = 0;
            // circle_pose1.pose.orientation.y = 0;
            // circle_pose1.pose.orientation.z = 0;
            // circle_pose1.pose.orientation.w = 1;

            // Rcircle_pos_pub.publish(circle_pose1);

            // int idx2 = valid_circles[1].first;
            // Eigen::VectorXf model2 = circle_models[idx2];
            // geometry_msgs::PoseStamped circle_pose2;
            // circle_pose2.header.stamp = ros::Time::now();
            // circle_pose2.header.frame_id = "world";
            // circle_pose2.pose.position.x = model2[0];
            // circle_pose2.pose.position.y = model2[1];
            // circle_pose2.pose.position.z = model2[2];
            // circle_pose2.pose.orientation.x = 0;
            // circle_pose2.pose.orientation.y = 0;
            // circle_pose2.pose.orientation.z = 0;
            // circle_pose2.pose.orientation.w = 1;
            // Lcircle_pos_pub.publish(circle_pose2);
...
pcl::PointCloud<pcl::PointXYZ>::Ptr cloudPreProcess(pcl::PointCloud<pcl::PointXYZ> cloud_in,int task){
    ...
    if(task == 2){
        ...
        x_max = current_pos.x() + 5;
        y_min = current_pos.y() + 1;
        y_max = current_pos.y() + 8; 
        z_min = 0.8;
        z_max = 3.8;
    }else if(task == 4){
        x_min = current_pos.x() + 0;
        ...
        y_min = current_pos.y() - 5;
        y_max = current_pos.y() + 0; 
        z_min = 1.8;
        z_max = 3.0;
    }else if(task == 7){
        x_min = current_pos.x() - 5;
...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    ...
    if(currentTask == 2){
        ...
        // }
        // 拟合平面
        Eigen::Vector4d plane = ransacFitOnePlane(points,*cloud_filtered, MaxIteration, threshold,GoalInliners,currentTask);
        
        std::cout << "Best fit plane "<<" : " << plane[0] << "x + " << plane[1] << "y + " << plane[2] << "z + " << plane[3] << " = 0" << std::endl;
        ...
        int NumOfPoints = cloud_filtered->size();
        std::cout<<"NumOfPoints: "<<cloud_filtered->size()<<std::endl;
        float MinProbInliers = 0.3;
        int MaxIteration = ceil(log(1 - 0.99) / log(1 - pow(MinProbInliers, 3)));//最大迭代轮数
        // MaxIteration = 200000;
        std::cout<<"MaxIteration: "<<MaxIteration<<std::endl;
        ...
        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered = cloudPreProcess(points_pcl,currentTask);
        ransacFitCircle(cloud_filtered,currentTask);
...
void orderCallback(const std_msgs::Int64::ConstPtr& msg) {
...
int main(int argc, char* argv[]) {
    ...
    ros::Subscriber order_subscriber = n.subscribe("/perc_mode", 10, orderCallback);
    ...
    circle1pub = n.advertise<sensor_msgs::PointCloud2>("/circle1", 10);
    circle2pub = n.advertise<sensor_msgs::PointCloud2>("/circle2", 10);
    tagPospub = n.advertise<geometry_msgs::PoseStamped>("/target_pose", 10);
    Lcircle_pos_pub = n.advertise<geometry_msgs::PoseStamped>("/Lcircle_pos", 10);
    Rcircle_pos_pub = n.advertise<geometry_msgs::PoseStamped>("/Rcircle_pos", 10);
    task2Pospub = n.advertise<nav_msgs::Path>("/task2_pose", 10);
    task3Pospub = n.advertise<nav_msgs::Path>("/task3_pose", 10);
