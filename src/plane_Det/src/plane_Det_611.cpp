...
ros::publisher left_circle_pub, right_circle_pub;
...
void estDoorPos(pcl::PointCloud<pcl::PointXYZ> cloud_in,int task){
    ...
    if(task == 2){
        ...
        double wallWidthTrue = 3.56;
        double wallHightTrue = 1.3;
        double doorWidthPos_NearLeft = 0.58;
        double doorHightPos_NearFloor = 0.7;
        ...
        }else if(detPlane_width>0.95*2.60 && detPlane_width <1.05*2.60){
...
Eigen::Vector4d ransacFitOnePlane(std::vector<Point>& points_input,pcl::PointCloud<pcl::PointXYZ> points_pcl_input, int iterations, double threshold,int GoalInliners,int task) {
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        //根据先验位置进行修改
        if(task == 2){
            if(-plane[3]/plane[0]>(19.5)*1.2 ||-plane[3]/plane[0]<(19.5)*0.8 || abs(plane[1]/plane[3])>0.1 || abs(plane[2]/plane[3])>0.1){
                // std::cout<<"A: "<<-plane[0]/plane[3]<<std::endl;
                continue;
            }
        }else if(task == 3){
...
void ransacFitCircle(pcl::PointCloud<pcl::PointXYZ>::Ptr points, int task, const std::string& side = ""){
        ///存放拟合圆
    ...
    if(task == 4){
        ...
        if(abs(coefficients->values[5])>0.95 && abs(coefficients->values[4])<0.2 && abs(coefficients->values[6])<0.5){
            ...
            std::cout << side << " circle pos:" << coefficients->values[0] << " " 
            ...
            // 根据左右侧选择发布器
            if (side == "left") {
                left_circle_pub.publish(circle_pose);
            } else if (side == "right") {
                right_circle_pub.publish(circle_pose);
            }
        }
        }
    }
...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    ...
    if(currentTask == 2){
        ...
        float y_center = current_pos.y();
        
        for (const auto& point : *cloud_filtered) {
            if (point.y < y_center) {  // 左侧点云
        ...
        ROS_INFO("Left cloud: %zu points, Right cloud: %zu points", 
                 left_cloud->size(), right_cloud->size());
        
        // 分别拟合左右点云中的圆环
        if (!left_cloud->empty()) {
            ransacFitCircle(left_cloud, currentTask, "left");
        }
        if (!right_cloud->empty()) {
            ransacFitCircle(right_cloud, currentTask, "right");
        }
    }
    auto frame_end_time = std::chrono::high_resolution_clock::now();
...
int main(int argc, char* argv[]) {
    ...
    left_circle_pub = nh.advertise<geometry_msgs::PoseStamped>("/left_circle_pose", 10);
    right_circle_pub = nh.advertise<geometry_msgs::PoseStamped>("/right_circle_pose", 10);
