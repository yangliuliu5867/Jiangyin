...
namespace path_search {
    ...
    RET_CODE Astar::pointToPointPathSearch(const rog_map::Vec3f &start_pt, const rog_map::Vec3f &end_pt,
        ...
        while (!open_set.empty()) {
            ...
            // std::cout<<"[A* debug]Not reached Horizon or Goal, but while works well"<<std::endl;
            ...
            if (!cfg_.visual_process && (time_2 - time_1) > time_out) {
                ...
                std::cout<<"searched distance: "<<current->distance_score<<" and the max is "<<searching_horizon / md_.resolution<<std::endl;
