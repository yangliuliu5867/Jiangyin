...
void geometry_utils::findBetterInterior(const Eigen::MatrixX4d& hPoly,
                                        Eigen::Vector3d& interior,
                                        const Eigen::Matrix3Xd& vPoly) {
    // 如果顶点计算失败，我们无法进行优化，直接返回，保留原始的interior点。
    if (vPoly.cols() == 0) {
        return;
    }

    // === 步骤 1: 获取 r_max ===
    Eigen::Vector3d temp_interior;
    const double r_max = findInteriorDist(hPoly, temp_interior);
    if (r_max <= 1e-6) {
        return;
    }
    // === 步骤 2: 计算几何中心 p_geom ===
    const Eigen::Vector3d p_geom = vPoly.rowwise().mean();
    // === 步骤 3: 求解最接近 p_geom 的最优球心 (LP2) ===
    const int m = hPoly.rows();
    const Eigen::ArrayXd hNorm = hPoly.leftCols<3>().rowwise().norm();
    const Eigen::MatrixX3d An = hPoly.leftCols<3>().array().colwise() / hNorm;
    const Eigen::VectorXd bn = -hPoly.rightCols<1>().array() / hNorm;

    const int num_vars_lp2 = 6;
    Eigen::Matrix<double, num_vars_lp2, 1> c_lp2(num_vars_lp2,1);
    c_lp2.head<3>().setZero();
    c_lp2.tail<3>().setOnes();

    const int num_constraints_lp2 = m + 6;
    Eigen::Matrix<double, Eigen::Dynamic, num_vars_lp2>  A_lp2(num_constraints_lp2, num_vars_lp2);
    Eigen::Matrix<double, Eigen::Dynamic, 1>  b_lp2(num_constraints_lp2, 1);
    A_lp2.setZero();

    // 约束1: 球心可行域 An * p <= bn - r_max
    const double safe_r_max = r_max - 1e-9;
    A_lp2.block(0, 0, m, 3) = An;
    b_lp2.head(m) = bn.array() - safe_r_max;

    // 约束2: L1范数 |p - p_geom| <= u
    A_lp2.block(m, 0, 3, 3) = Eigen::Matrix3d::Identity();
    A_lp2.block(m, 3, 3, 3) = -Eigen::Matrix3d::Identity();
    b_lp2.segment(m, 3) = p_geom;

    A_lp2.block(m + 3, 0, 3, 3) = -Eigen::Matrix3d::Identity();
    A_lp2.block(m + 3, 3, 3, 3) = -Eigen::Matrix3d::Identity();
    b_lp2.segment(m + 3, 3) = -p_geom;

    Eigen::Matrix<double, num_vars_lp2, 1> x_lp2(num_vars_lp2, 1);
    math_utils::sdlp::linprog<6>(c_lp2, A_lp2, b_lp2, x_lp2); // Ensure correct template parameters

    // === 步骤 4: 更新 interior 变量 ===
    // 用我们找到的更好的点，覆盖掉原始的输入点。
    interior = x_lp2.head<3>();
}
