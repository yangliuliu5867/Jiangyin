...
bool ExpTrajOpt::processCorridorWithGuideTraj() {
    ...
    for (int i = 0; i < sizeCorridor; i++) {
        ...
        /*LYX
        值得注意的是，super在此处有一个漏洞：得到的虽然是最大内切球心，但球心解不止一个，以无避障直线飞行（rest-to-rest）为例：
        此时，安全飞行空间走廊（SFC）会预设为box_max和box_min决定的长方体，此时大内切球有无穷多个，球心解为
        1. 一个平面（长宽高互不相同）
        2. 线段(长宽高中有两值相等)
        3. 点（正方体）
        可见，即使是长方体，在绝大多数情况下内切球解为无穷，妄论复杂凸多面体。而显而易见，我们的需求是尽可能靠近几何中心，而不是随机选取，否则轨迹会不可控地变形。
        这个问题暴露在窄框避障和无避障直线飞行两个情况：前者会因为A*的倾向而选取球心时偏向一侧，导致碰撞；后者则是随机选点导致多个点间互不共线。
        此处做一个改进，利用已经得到的curIV顶点集合得到几何中心interior = curIV.rowwise().mean(); 
        再进一步得到离几何中心最近的球心。
        LYX 2025/6/27 */
        geometry_utils::findBetterInterior(curIH, interior, curIV);
...
bool ExpTrajOpt::optimize(const StatePVAJ &headPVAJ, const StatePVAJ &tailPVAJ,
    ...
    if (!success && cfg_.save_log_en) {
        failed_traj_log << 990419 << endl;//shen me si huo
        failed_traj_log << headPVAJ << endl;
        failed_traj_log << tailPVAJ << endl;
