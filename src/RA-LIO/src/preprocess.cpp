...
{
  pl_surf.clear(); //把面点点云清空
  
  //下面为定义RS类型的点云，将msg转为RS类型pl_orig，然后记录size，保存下来
  ...
  for (int i = 0; i < plsize; i++)
  {
    PointType added_pt;  //PCL下点云的标准类型

    added_pt.normal_x = 0; //三个方向的法向量
    added_pt.normal_y = 0;
    added_pt.normal_z = 0;
    added_pt.x = pl_orig.points[i].x; //三个方向的位置，强度，曲率（此处并不是真的曲率，而是一个相对时间戳ms）
    added_pt.y = pl_orig.points[i].y;
    added_pt.z = pl_orig.points[i].z;
    added_pt.intensity = pl_orig.points[i].intensity;
