...
namespace ego_planner
{
  ...
  private:
    ...
    traj_utils::FlagState flag_state_;

    /* parameters */
    int target_type_; // 1 mannual select, 2 preset running, 3 reference path, 4 flag fsm
    double no_replan_thresh_, replan_thresh_;
    double normal_speed_;
    double slow_speed_;
    double landing_speed_;
    double waypoints_[1000][3];                // 4:store all waypoint received in callback
    int waypoint_received_[1000];              // store if the waypoint is received
    int way_point_mode_[1000];                 // store each point mode
    int waypoint_num_, wpt_id_, running_mode; // total waypoint and now waypoint id
    ...
    ros::Subscriber waypoint_sub_, odom_sub_, trigger_sub_, broadcast_ploytraj_sub_, mandatory_stop_sub_, flag_sub_;
    ros::Publisher poly_traj_pub_, data_disp_pub_, broadcast_ploytraj_pub_, heartbeat_pub_, ground_height_pub_, flag_state_pub_;
    ...
    void flagStartFirstPoint();
    void flagStatePub(Eigen::Vector3d pos,int id, int wp_num, int mode, int is_touch,bool is_in_obstacle);
    ...
    void flagCallback(const traj_utils::FlagConstPtr &msg);
